...
//	lcdc_dev->pd   = devm_clk_get(lcdc_dev->dev, "pd_lcdc");
	...
	if (lcdc_dev->pwr18 == true)
		...
		v = 0x00010000;
	ret = regmap_write(lcdc_dev->grf, RK3288_GRF_IO_VSEL, val);
	if (ret != 0) {
		dev_err(lcdc_dev->dev, "Could not write to GRF: %d\n", ret);
		return ret;
	}
#endif
...
static int rk3288_load_screen(struct rk_lcdc_driver *dev_drv, bool initscreen)
{
	...
	if (likely(lcdc_dev->clk_on)) {
		...
		lcdc_msk_reg(lcdc_dev, SYS_CTRL, mask, val);
#ifndef CONFIG_RK_FPGA
		ret = regmap_write(lcdc_dev->grf, RK3288_GRF_SOC_CON6, val);
		if (ret != 0) {
			dev_err(lcdc_dev->dev, "Could not write to GRF: %d\n", ret);
		}
#endif		
			...
			struct rk_screen *screen, struct rk_lcdc_win *win)
{
	...
	DBG(1, "lcdc%d>>%s\n>>>xact:%d>>yact:%d>>xsize:%d>>ysize:%d\n"
		">>xvir:%d>>yvir:%d>>xpos:%d>>ypos:%d>>\n", lcdc_dev->id,
		__func__, xact,	yact, win->area[0].xsize, win->area[0].ysize, xvir, yvir, xpos, ypos);
	return 0;
...
static int rk3288_lcdc_set_dsp_cabc(struct rk_lcdc_driver *dev_drv, int mode)
{
	...
	ret = regmap_write(lcdc_dev->grf, RK3288_GRF_GPIO7A_IOMUX, val);
	if (ret != 0) {
		dev_err(lcdc_dev->dev, "Could not write to GRF: %d\n", ret);
		return ret;
	}
...
static int rk3288_lcdc_probe(struct platform_device *pdev)
{
	...
	lcdc_dev->grf = syscon_regmap_lookup_by_phandle(np, "rockchip,grf");
	if (IS_ERR(lcdc_dev->grf)) {
		dev_err(dev,
				"rk3288-edp needs rockchip,grf property\n");
		return PTR_ERR(lcdc_dev->grf);
	}
